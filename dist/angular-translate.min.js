...
/**
   ...
   *
   */
b={sanitize:function(a,b){return"text"===b&&(a=g(a)),a},escape:function(a,b){return"text"===b&&(a=f(a)),a},sanitizeParameters:function(a,b){return"params"===b&&(a=h(a,g)),a},escapeParameters:function(a,b){return"params"===b&&(a=h(a,f)),a}},
...
b.escaped=b.escapeParameters,/**
   ...
   * @returns {object} this
   */
this.addStrategy=function(a,c){return b[a]=c,this},/**
   * @ngdoc function
   ...
   * @returns {object} this
   */
this.removeStrategy=function(a){return delete b[a],this},/**
   * @ngdoc function
   ...
   * @returns {object} this
   */
this.useStrategy=function(a){return d=!0,c=a,this},/**
   * @ngdoc object
   ...
   *
   */
this.$get=["$injector","$log",function(f,g){var h={},i=function(a,c,d){return angular.forEach(d,function(d){if(angular.isFunction(d))a=d(a,c);else if(angular.isFunction(b[d]))a=b[d](a,c);else{if(!angular.isString(b[d]))throw new Error("pascalprecht.translate.$translateSanitization: Unknown sanitization strategy: '"+d+"'");if(!h[b[d]])try{h[b[d]]=f.get(b[d])}catch(e){throw h[b[d]]=function(){},new Error("pascalprecht.translate.$translateSanitization: Unknown sanitization strategy: '"+d+"'")}a=h[b[d]](a,c)}}),a},j=function(){d||e||(g.warn("pascalprecht.translate.$translateSanitization: No sanitization strategy has been configured. This can have serious security implications. See http://angular-translate.github.io/docs/#/guide/19_security for details."),e=!0)};return f.has("$sanitize")&&(a=f.get("$sanitize")),{/**
       * @ngdoc function
       ...
       * @param {string|StrategyFunction|array} strategy The sanitization strategy / strategies which should be used. Either a name of an existing strategy, a custom strategy function, or an array consisting of multiple names and / or custom functions.
       */
useStrategy:function(a){return function(b){a.useStrategy(b)}}(this),/**
       * @ngdoc function
       ...
       */
sanitize:function(a,b,d){if(c||j(),arguments.length<3&&(d=c),!d)return a;var e=angular.isArray(d)?d:[d];return i(a,b,e)}}}];var f=function(a){var b=angular.element("<div></div>");// not chainable, see #1044
...
var J=function(){var a=I()||"";return G[F]&&(a=G[F](a)),a};J.displayName="angular-translate/service: getLocale";/**
   ...
   */
var K=function(a,b){for(var c=0,d=a.length;c<d;c++)if(a[c]===b)return c;return-1},L=function(){return this.toString().replace(/^\s+|\s+$/g,"")},M=function(a){if(a){for(var b=[],c=angular.lowercase(a),d=0,e=v.length;d<e;d++)b.push(angular.lowercase(v[d]));
...
this.nestedObjectDelimeter=function(a){return a?(B=a,this):B};/**
   ...
   */
var O=function(a,b,c,d){var e,f,g,h;b||(b=[]),c||(c={});for(e in a)Object.prototype.hasOwnProperty.call(a,e)&&(h=a[e],angular.isObject(h)?O(h,b.concat(e),c,e):(f=b.length?""+b.join(B)+B+e:e,b.length&&e===d&&(
// Create shortcut path (foo.bar == foo.bar.bar)
g=""+b.join(B),
// Link it to original path
c[g]="@:"+f),c[f]=h));return c};O.displayName="flatObject",/**
   * @ngdoc function
...
return U(a,b,c,h,j).then(g,g),f.promise},i=0,k=a.length;i<k;i++)f.push(g(a[i]));
...
return q&&(a=[q,a].join(" ")),r&&(a=[a,r].join(" ")),a},W=function(a){i=a,
// make sure to store new language key before triggering success event
k&&f.put(U.storageKey(),i),c.$emit("$translateChangeSuccess",{language:a}),
// inform default interpolator
G.setLocale(i);var b=function(a,b){S[b].setLocale(i)};b.displayName="eachInterpolatorLocaleSetter",
...
// Start with the fallbackLanguage with index 0
return ca(F>0?F:l,a,b,c)},fa=function(a,b,e,f,h){"undefined"==typeof c.translation_variables&&(c.translation_variables={}),console.log(a),a in c.translation_variables||(c.translation_variables[a]=!0);var i=d.defer(),j=h?u[h]:u,k=e?S[e]:G;
// if the translation id exists, we can just interpolate it
if(j&&Object.prototype.hasOwnProperty.call(j,a)){var l=j[a];
// If using link, rerun $translate with linked translationId and return it
if("@:"===l.substr(0,2))U(l.substr(2),b,e,f,h).then(i.resolve,i.reject);else{
//
var n=k.interpolate(l,b);n=ia(a,l,n,b,h),i.resolve(n)}}else{var o;
// for logging purposes only (as in $translateMissingTranslationHandlerLog), value is not returned to promise
m&&!I&&(o=aa(a,b,f)),
// since we couldn't translate the inital requested translation id,
// we try it now with one or more fallback languages, if fallback language(s) is
// configured.
h&&g&&g.length?da(a,b,k,f).then(function(a){i.resolve(a)},function(a){i.reject(V(a))}):m&&!I&&o?
// looks like the requested translation id doesn't exists.
// Now, if there is a registered handler for missing translations and no
// asyncLoader is pending, we execute the handler
f?i.resolve(f):i.resolve(o):f?i.resolve(f):i.reject(V(a))}return i.promise},ga=function(a,b,d,e){"undefined"==typeof c.translation_variables&&(c.translation_variables={}),console.log(a),a in c.translation_variables||(c.translation_variables[a]=!0);var f,h=e?u[e]:u,i=G;
// if the translation id exists, we can just interpolate it
if(
// if the interpolation id exists use custom interpolator
S&&Object.prototype.hasOwnProperty.call(S,d)&&(i=S[d]),h&&Object.prototype.hasOwnProperty.call(h,a)){var j=h[a];
// If using link, rerun $translate with linked translationId and return it
f="@:"===j.substr(0,2)?ga(j.substr(2),b,d,e):i.interpolate(j,b)}else{var k;
// for logging purposes only (as in $translateMissingTranslationHandlerLog), value is not returned to promise
m&&!I&&(k=aa(a,b)),
// since we couldn't translate the inital requested translation id,
// we try it now with one or more fallback languages, if fallback language(s) is
// configured.
e&&g&&g.length?(l=0,f=ea(a,b,i)):
// looks like the requested translation id doesn't exists.
// Now, if there is a registered handler for missing translations and no
// asyncLoader is pending, we execute the handler
f=m&&!I&&k?k:V(a)}return f},ha=function(a){j===a&&(j=void 0),T[a]=void 0},ia=function(a,c,d,e,f){var g=t;
...
if(Q(a),o&&g&&g.length)for(var b=0,c=g.length;b<c;b++)T[g[b]]||(T[g[b]]=X(g[b]));U.use(U.use())}return h?g[0]:g},/**
       * @ngdoc function
...
return v.length>0&&!e?d.reject(a):(e&&(a=e),j=a,!A&&u[a]||!o||T[a]?T[a]?T[a].then(function(a){return j===a.key&&W(a.key),b.resolve(a.key),a},function(a){
// find first available fallback language if that request has failed
// find first available fallback language if that request has failed
return!i&&g&&g.length>0?U.use(g[0]).then(b.resolve,b.reject):b.reject(a)}):(b.resolve(a),W(a)):(T[a]=X(a).then(function(c){return N(c.key,c.table),b.resolve(c.key),j===a&&W(c.key),c},function(a){return c.$emit("$translateChangeError",{language:a}),b.reject(a),c.$emit("$translateChangeEnd",{language:a}),d.reject(a)}),T[a]["finally"](function(){ha(a)})),b.promise)},/**
       * @ngdoc function
...
i&&!k[i]&&j.push(X(i));var n=function(a){u={},angular.forEach(a,function(a){N(a.key,a.table)}),i&&W(i),b()};n.displayName="refreshPostProcessor",d.all(j).then(n,e)}return f.promise},/**
       ...
       */
U.instant=function(a,b,c,d){console.log(a);
...
if(
// Check forceLanguage is present
d&&ja(d),angular.isArray(a)){for(var h={},j=0,k=a.length;j<k;j++)h[a[j]]=U.instant(a[j],b,c,d);return h}
// We discarded unacceptable values. So we just need to verify if translationId is empty String
if(angular.isString(a)&&a.length<1)return a;
// trim off any whitespace
a&&(a=L.apply(a));var l,n=[];e&&n.push(e),f&&n.push(f),g&&g.length&&(n=n.concat(g));for(var o=0,p=n.length;o<p;o++){var s=n[o];if(u[s]&&"undefined"!=typeof u[s][a]&&(l=ga(a,b,c,f)),"undefined"!=typeof l)break}
...
angular.equals(u,{})&&U.use()&&U.use(U.use()),g&&g.length)for(var na=function(a){return N(a.key,a.table),c.$emit("$translateChangeEnd",{language:a.key}),a},oa=0,pa=g.length;oa<pa;oa++){var qa=g[oa];!A&&u[qa]||(T[qa]=X(qa).then(na))}}else c.$emit("$translateReady",{language:U.use()});return U}]}function d(a,b){"use strict";var c,d={},e="default";/**
   * @ngdoc function
